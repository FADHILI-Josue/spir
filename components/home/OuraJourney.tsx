...
const OuraJourney: FC<OuraJourneyProps> = ({ }) => {
    ...
    return <div className='relative flex h-[100vh] w-full flex-col items-center justify-center bg-[#E6DED3]' ref={topRef}>
        {/* {scroll} */}
        <div className="absolute left-10 md:left-32 z-30 space-y-8 self-start">
            <h1 className='max-w-md text-3xl lg:text-6xl  font-light leading-tight text-white'> Begin your Spir journey <em className='font-serif'>today</em>.</h1>
            <Button className='text-md rounded-full bg-white px-10 py-7 font-normal text-black hover:bg-[#D0ECF5]'>Shop Now</Button>
        </div>
        <div className='h-[99%] border border-black w-[96%]  overflow-hidden'>
            ...
            <Image src={ringOnRing} style={{ transform: `translate(-2rem, ${isLargeScreen ? scroll: scroll}px)`, objectFit: 'cover', width: '100%', height: '100%', scale: '1.1', zIndex: '-10' }} className='-translate-x-32 sm:translate-x-0 h-screen' alt='ring of Spir' />
        </div>
    </div>
}
